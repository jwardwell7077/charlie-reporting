...
def temp_dirs():
    ...
    with tempfile.TemporaryDirectory() as temp_root:
        save_dir = os.path.join(temp_root, 'save')
        scan_dir = os.path.join(temp_root, 'incoming')
...
class TestDirectoryScanning:
    ...
    def test_directory_scan_for_date(self, config_with_directory_scan, temp_dirs, sample_csv_files):
        ...
        ib_calls_files = [f for f in saved_files if f.startswith('IB_Calls__')]
        assert len(ib_calls_files) == 1
        
        dials_files = [f for f in saved_files if f.startswith('Dials__')]
    ...
    def test_directory_scan_for_recent(self, config_with_directory_scan, temp_dirs, sample_csv_files):
        ...
        dials_files = [f for f in saved_files if f.startswith('Dials__')]
        test_files = [f for f in saved_files if f.startswith('Test_Report__')]
    ...
    def test_directory_scan_disabled(self, config_without_directory_scan, temp_dirs, sample_csv_files):
        ...
        # Mock the _scan_directory_for_date method to verify it's not called
        fetcher._scan_directory_for_date = Mock()
        ...
        fetcher._scan_directory_for_date.assert_not_called()
    ...
    def test_directory_scan_nonexistent_path(self, config_with_directory_scan, temp_dirs):
        ...
        fetcher._scan_directory_for_date('2025-07-28')
    ...
    def test_directory_scan_with_subdirs(self, config_with_directory_scan, temp_dirs):
        ...
        sub_file = os.path.join(subdir, 'IB_Calls.csv')
...
class TestOutlookAccountSelection:
    ...
    def test_get_inbox_for_specific_account(self, mock_dispatch, config_with_directory_scan):
        ...
        mock_account1.DeliveryStore.GetDefaultFolder.return_value = Mock()
        
        mock_account2 = Mock()
        mock_account2.SmtpAddress = 'test@company.com'  # Matches config
        mock_target_inbox = Mock()
        mock_account2.DeliveryStore.GetDefaultFolder.return_value = mock_target_inbox
        ...
        mock_namespace.GetDefaultFolder.return_value = Mock()  # Default inbox
        ...
        inbox = fetcher._get_inbox_for_account(mock_namespace)
    ...
    def test_get_inbox_fallback_to_default(self, mock_dispatch, config_with_directory_scan):
        ...
        mock_default_inbox = Mock()
        mock_namespace.GetDefaultFolder.return_value = mock_default_inbox
...
class TestIntegratedFunctionality:
    ...
    def test_fetch_with_directory_scan_enabled(self, mock_dispatch, config_with_directory_scan, temp_dirs, sample_csv_files):
        ...
        mock_inbox = Mock()
        mock_messages = Mock()
        mock_messages.__iter__ = Mock(return_value=iter([]))  # No messages
        ...
        fetcher._scan_directory_for_date.assert_called_once_with('2025-07-28')
    ...
    def test_fetch_hourly_with_directory_scan(self, mock_dispatch, config_with_directory_scan, temp_dirs):
        ...
        mock_inbox = Mock()
        mock_messages = Mock()
        mock_messages.__iter__ = Mock(return_value=iter([]))
        mock_messages.Sort = Mock()
        ...
        fetcher._scan_directory_for_hour = Mock()
        
        # Call fetch_hourly
        fetcher.fetch_hourly('2025-07-28', 14)
        
        # Verify directory scan for hour was called
        fetcher._scan_directory_for_hour.assert_called_once_with('2025-07-28', 14)
    ...
    def test_fetch_recent_with_directory_scan(self, mock_dispatch, config_with_directory_scan, temp_dirs):
        ...
        fetcher._scan_directory_for_recent = Mock()
        
        # Call fetch_recent
        fetcher.fetch_recent(2)
        
        # Verify directory scan for recent was called
        fetcher._scan_directory_for_recent.assert_called_once_with(2)
