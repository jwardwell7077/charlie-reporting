...
def test_generate_invalid_dataset(client: TestClient) -> None:
...
def test_download_404(client: TestClient) -> None:
    resp = client.get("/sim/download/DOES_NOT_EXIST.csv")
    assert resp.status_code == 404
    assert "File not found" in resp.json()["detail"]

def test_spec_addendum_not_found(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    # Patch Path.is_file to always return False
    from pathlib import Path
    monkeypatch.setattr(Path, "is_file", lambda self: False)  # type: ignore[no-untyped-def]
