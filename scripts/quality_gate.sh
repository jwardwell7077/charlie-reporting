...
run "Ruff"      "${VENV_BIN}/ruff" check "${SRC_DIR}"
...
run "Pyright"     "${VENV_BIN}/pyright" --warnings "${SRC_DIR}"
run "Pytest"    env PYTHONPATH="${SRC_DIR}" "${VENV_BIN}/pytest" -vv foundation/tests
