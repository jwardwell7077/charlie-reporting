...
class LegacyMigrator:
    ...
    def __init__(self, project_root: str):
        ...
        self.backup_dir = self.project_root / "legacy_backup" / datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    def migrate_shared_utilities(self):
        ...
        for old_file, new_file in migrations:
            old_path = self.src_dir / old_file
            new_path = self.shared_dir / new_file
            
            if old_path.exists():
                print(f"  📦 {old_file} → shared/{new_file}")
                shutil.copy2(old_path, new_path)
                self._update_imports_in_file(new_path, old_file)
            else:
...
'''
        ...
        bridge_path = self.project_root / "legacy_bridge.py"
    ...
    def _update_imports_in_file(self, file_path: Path, original_name: str):
    ...
    def create_migration_report(self):
        ...
        report_content = f"""# Legacy Code Migration Report
...
"""
    ...
    def validate_migration(self):
        ...
        bridge_exists = (self.project_root / "legacy_bridge.py").exists()
