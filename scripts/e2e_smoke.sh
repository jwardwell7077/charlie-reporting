...
PIDS=""
# Start services
SP_PID=$(start_server sim python -m uvicorn src.sharepoint_sim.server:app --reload --port 8001)
PIDS+=" $SP_PID"
DB_PID=$(start_server db  python -m uvicorn src.db_service_api:app        --reload --port 8000)
PIDS+=" $DB_PID"
RS_PID=$(start_server rpt python -m uvicorn src.report_service_api:app    --port   8091)
PIDS+=" $RS_PID"
...
SIM_BASE_URL="$SIM_BASE_URL" DB_API_URL="$DB_API_URL" REPORT_API_URL="$REPORT_API_URL" \
  python scripts/preflight_health.py

# Run E2E once
echo "\nRunning E2E once ..."
SIM_BASE_URL="$SIM_BASE_URL" DB_API_URL="$DB_API_URL" REPORT_API_URL="$REPORT_API_URL" \
  python scripts/run_e2e_once.py
