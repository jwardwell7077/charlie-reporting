...
def _settings(tmp: Path) -> Settings:
	return Settings(
		...
		collector=CollectorConfig(
			input_root=tmp / "incoming",
			staging_dir=tmp / "staging",
			archive_dir=tmp / "archive",
		),
		report=ReportConfig(
			output_dir=tmp / "reports",
			workbook_name="r.xlsx",
			columns={
...
def test_load_csv_into_table_ignores_missing_columns(tmp_path: Path) -> None:
	db = tmp_path / "db.sqlite"
	init_db(db)
	# CSV lacks 'revenue' column specified; values become None
	csv_path = tmp_path / "p.csv"
	csv_path.write_text("agent,date,acq\na,2025-01-01T00:00:00,5\n")
	rows = load_csv_into_table(db, csv_path, "productivity", ["agent", "date", "acq", "revenue"])
	assert rows == 1
	conn = sqlite3.connect(db)
	try:
		cur = conn.execute("SELECT revenue FROM productivity LIMIT 1")
		(rev,) = cur.fetchone()
		assert rev is None
	finally:
		conn.close()
