...
def test_fetch_for_timeframe_full_day(sample_config, temp_save_dir, mock_outlook, mock_outlook_message):
    ...
    mock_outlook.__iter__ = Mock(return_value=iter([mock_outlook_message]))
...
def test_fetch_for_timeframe_specific_hours(sample_config, temp_save_dir, mock_outlook):
    ...
    mock_outlook.__iter__ = Mock(return_value=iter([msg_14, msg_15, msg_16]))
    
    fetcher = EmailFetcher(sample_config, save_dir=temp_save_dir)
    
    # Mock the _is_valid_email method to return True for our test messages
    fetcher._is_valid_email = Mock(return_value=True)
    
    # Fetch only hour 15
    fetcher.fetch_for_timeframe('2025-07-27', start_hour=15, end_hour=15)
    
    # Should have processed only the 15:30 message
    assert fetcher._is_valid_email.call_count == 1  # Only msg_15 should be validated
...
def test_fetch_hourly(sample_config, temp_save_dir, mock_outlook):
    ...
    fetcher.fetch_for_timeframe = Mock()
...
def test_fetch_recent(sample_config, temp_save_dir, mock_outlook):
    ...
    mock_outlook.__iter__ = Mock(return_value=iter([msg_recent, msg_old]))
    
    fetcher = EmailFetcher(sample_config, save_dir=temp_save_dir)
    fetcher._is_valid_email = Mock(return_value=True)
    fetcher._get_attachment_rule = Mock(return_value={'columns': ['Agent Name']})
...
def test_filename_with_timestamp(sample_config, temp_save_dir, mock_outlook):
    ...
    mock_outlook.__iter__ = Mock(return_value=iter([msg]))
    ...
    call_args = mock_attachment.SaveAsFile.call_args[0][0]
...
def test_duplicate_file_handling(sample_config, temp_save_dir, mock_outlook):
    ...
    existing_file = os.path.join(temp_save_dir, 'IB_Calls__2025-07-27_1430.csv')
...
def test_email_processing_error(sample_config, temp_save_dir, mock_outlook):
    ...
    msg.Attachments.Item.side_effect = Exception("Attachment error")
...
def test_filter_validation_integration(sample_config, temp_save_dir, mock_outlook):
    ...
    mock_outlook.__iter__ = Mock(return_value=iter([valid_msg, invalid_sender, invalid_subject]))
    ...
    def track_validation(msg, filters):
        result = fetcher._is_valid_email.__wrapped__(fetcher, msg, filters)
        if result:
            processed_messages.append(msg)
        return result
    
    original_is_valid = fetcher._is_valid_email
    fetcher._is_valid_email = Mock(side_effect=track_validation)
    
    fetcher.fetch_for_timeframe('2025-07-27')
    
    # Should have validated all 3 messages but only processed the valid one
    assert fetcher._is_valid_email.call_count == 3
