...
class ExcelReportService:
        ...
        for sheet_name, sheet in report.sheets.items():
            ...
            if sheet.row_count > 100000:
                validation["warnings"].append(
                    f"Sheet '{sheet_name}' has {sheet.row_count} rows - may cause performance issues"
                )
    ...
    def create_incremental_update_strategy(self, existing_file: Path | None, new_report: Report) -> dict[str, Any]:
...
class ExcelGeneratorService(IExcelGenerator):
    ...
    async def apply_formatting(self, workbook_data: bytes, rules: dict[str, Any]) -> bytes:
    ...
    async def add_sheet(self, workbook_data: bytes, sheet_name: str, data: Any) -> bytes:
