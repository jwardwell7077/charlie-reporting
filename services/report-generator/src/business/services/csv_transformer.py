...
class CSVTransformationService:
    ...
    def discover_csv_files(self, raw_dir: Path, date_filter: str, hour_filter: Optional[str] = None) -> List[CSVFile]:
        ...
        for file_path in raw_dir.iterdir():
            ...
            # Check date filter
            if f"__{date_filter}" not in file_name:
                self.logger.debug(f"Skipping {file_name}: date mismatch")
                continue

            # Check hour filter if specified
            if hour_filter:
                hour_pattern = f"__{date_filter}_{hour_filter}"
                if hour_pattern not in file_name:
                    self.logger.debug(f"Skipping {file_name}: hour mismatch")
                    continue

            # Extract hour from file_name
            hour_str = self.extract_hour_from_filename(file_name)
    ...
    def match_files_to_rules(self, csv_files: List[CSVFile], rules: List[CSVRule]) -> List[CSVFile]:
        ...
        for csv_file in csv_files:
            ...
            if matching_rule:
                ...
                self.logger.warning(f"No matching rule for: {csv_file.file_name}")
    ...
    def transform_csv_file(self, csv_file: CSVFile) -> CSVTransformationResult:
        ...
        try:
            ...
            self.logger.debug(f"Read CSV {csv_file.file_name}: {len(df)} rows, {len(df.columns)} columns")
    ...
    def archive_processed_files(self,
        ...
        for result in successful_results:
            ...
            try:
                ...
                archive_path = archive_dir / result.file.file_name

                # Move file to archive
                shutil.move(str(source_path), str(archive_path))

                archived_files[result.file.file_name] = str(archive_path)
                self.logger.info(f"Archived file: {result.file.file_name}")

            except Exception as e:
                self.logger.error(f"Failed to archive {result.file.file_name}: {e}")
...
class CSVTransformerService(ICSVTransformer):
    ...
    async def transform_csv(
        ...
        try:
            ...
            result = self.service.transform_csv_file(csv_file)
