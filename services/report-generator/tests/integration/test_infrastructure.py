...
class IntegrationTestRunner:
    ...
    async def run_complete_workflow_test(
        ...
        try:
            ...
            # Validate results
            validationresults = self.validate_workflow_results(
    ...
    def validate_workflow_results(
        ...
        if result.output_file:
            outputpath = Path(result.output_file)
        ...
        expectedfiles = expected_outcomes.get('files_processed')
...
class PerformanceTestSuite:
            ...
            for record_count in record_counts:
                ...
                for i in range(iterations):
                    ...
                    with TestEnvironmentFactory() as env:
                        ...
                        # Measure performance
                        starttime = time.time()
                        startmemory = self.get_memory_usage()
                        ...
                        try:
                            ...
                            endtime = time.time()
                            endmemory = self.get_memory_usage()
                ...
                successfulruns = [r for r in iteration_results if r.get('success', False)]
    ...
    def generate_performance_summary(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        successfulresults = [r for r in results if r.get('success_rate', 0) > 0]
...
class ErrorInjectionTestSuite:
        ...
        scenarios = [
            {
                ...
                'setup': lambda: self.make_directory_readonly(test_environment['input_directory']),
                'expected_error': 'Permission denied'
            },
            {
                'name': 'disk_full',
                'setup': lambda: self.simulate_disk_full(test_environment['output_directory']),
                'expected_error': 'No space left'
            },
            {
                'name': 'corrupt_files',
                'setup': lambda: self.create_corrupt_csv_files(test_environment['input_directory']),
    ...
    def make_directory_readonly(self, directory: str):
    ...
    def simulate_disk_full(self, directory: str):
        """Simulate disk full condition"""
        # This would implement disk space simulation
        # For testing purposes, we might mock this
        pass

    def create_corrupt_csv_files(self, directory: str):
        """Create corrupt CSV files for testing"""
        corruptfile = Path(directory) / "corrupt__2025 - 07 - 29__0900.csv"
