...
class TestTimer:
    ...
    def stop(self):
        ...
        if self.start_time is not None:
            ...
            self.elapsedseconds = self.end_time - self.start_time
...
def with_test_timer(name: str = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            testname = name or func.__name__
...
class TestAssertions:
    ...
    def assert_processing_failure(result, expected_error: str = None):
        ...
        if expected_error:
            errortext = ' '.join(str(e) for e in result.errors)
    ...
    def assert_mock_called(mock_service, method_name: str, min_calls: int = 1):
        ...
        if hasattr(mock_service, 'get_call_count'):
            callcount = mock_service.get_call_count(method_name)
    ...
    def assert_data_integrity(original_data: Dict, processed_data: Dict):
        ...
        if 'columns' in original_data and 'columns' in processed_data:
            originalcolumns = set(original_data['columns'])
            processedcolumns = set(processed_data['columns'])
...
class TestReporting:
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        successfultests = [t for t in self.test_results if t['success']]
        failedtests = [t for t in self.test_results if not t['success']]

        totalduration = sum(t['duration'] for t in self.test_results)
        avgduration = total_duration / len(self.test_results) if self.test_results else 0
    ...
    def print_summary(self):
        ...
        print("="*50)  # Utility functions for common test operations
