...
class ExcelGeneratorImpl(IExcelGenerator):
    ...
    def create_workbook_sync(self, data: Dict[str, Any]) -> bytes:
        ...
        for sheet_name, sheet_data in data.items():
            ...
            if isinstance(sheet_data, pd.DataFrame):
                self.add_dataframe_to_sheet(ws, sheet_data)
            elif isinstance(sheet_data, list):
                self.add_list_data_to_sheet(ws, sheet_data)
            elif isinstance(sheet_data, dict):
                self.add_dict_data_to_sheet(ws, sheet_data)
            else:
    ...
    def add_dataframe_to_sheet(self, worksheet, df: pd.DataFrame) -> None:
    ...
    async def add_formatting_rules(self, workbook_bytes: bytes,
        ...
        formatted_bytes = await loop.run_in_executor(
            None,
            self.apply_formatting_rules_sync,
            workbook_bytes,
            rules
        )
