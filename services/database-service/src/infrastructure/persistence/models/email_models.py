...
class EmailRecordORM(Base, TimestampMixin):
    ...
    recipients = Column(JSON, nullable=False)  # Stored as JSON array
    ...
    def __repr__(self) -> str:  # pragma: no cover - convenience
        return (
            f"<EmailRecordORM(id={self.id}, message_id='{self.message_id}', "
...
class AttachmentORM(Base, TimestampMixin):
    ...
    size_bytes = Column(String(20), nullable=False)  # Stored as text
    file_path = Column(String(1000), nullable=True)
    content_id = Column(String(255), nullable=True)
    is_inline = Column(String(10), default="false")  # Store as string flag
    email_record_id = Column(PGUUID(as_uuid=True), nullable=True, index=True)

    def __repr__(self) -> str:  # pragma: no cover - convenience
        return (
            f"<AttachmentORM(id={self.id}, filename='{self.filename}', "
...
class ReportORM(Base, TimestampMixin):
    ...
    created_by_id = Column(PGUUID(as_uuid=True), nullable=False, index=True)
    email_record_ids = Column(JSON, default=list)  # JSON array of UUID strings
    file_path = Column(String(1000), nullable=True)
    completed_at = Column(DateTime(timezone=True), nullable=True)

    def __repr__(self) -> str:  # pragma: no cover - convenience
        return (
            f"<ReportORM(id={self.id}, title='{self.title}', "
