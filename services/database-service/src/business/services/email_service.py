...
Handles complex email processing workflows and business rules.
"""
...
class EmailService:
    ...
    def __init__(self,
        ...
        """
        self._logger.info(f"Importing email with message_id: {email_data.get('message_id')}")
        ...
        existing_email = await self._email_repository.find_by_message_id(
            ...
            if existing_email:
            self._logger.warning(f"Duplicate email detected: {email_data.get('message_id')}")
                raise ValueError(f"Email with message_id {email_data.get('message_id')} already exists")
        ...
        email_record = self._apply_business_rules(email_record)
            ...
            # Save to repository
        saved_email = await self._email_repository.save(email_record)
    ...
    async def process_email(self, email_id: UUID) -> EmailRecord:
        ...
        email = self._apply_processing_rules(email)
            ...
            # Save updated email
        processed_email = await self._email_repository.save(email)

            self._logger.info(f"Successfully processed email: {email_id}")
            return processed_email

    async def bulk_process_emails(self, email_ids: List[UUID]) -> Dict[UUID, bool]:
            """
        ...
        success_count = sum(results.values())
            self._logger.info(f"Bulk processing complete: {success_count}/{len(email_ids)} successful")
    ...
    def _apply_business_rules(self, email: EmailRecord) -> EmailRecord:
        ...
        sender_domain = email.sender.split('@')[-1] if '@' in email.sender else ''
            ...
            self._logger.debug(f"Applied high priority rule for domain: {sender_domain}")
        ...
        total_recipients = len(email.recipients) + len(email.cc_recipients)
            ...
            self._logger.warning(f"Large recipient list detected: {total_recipients} recipients")
