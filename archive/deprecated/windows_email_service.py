...
class Config:
    ...
    OUTPUT_DIR = Path(os.getenv("EMAIL_SERVICE_OUTPUT_DIR", "C:/EmailService/Output"))
...
class OutlookService:
    ...
    def fetch_emails(self, request: FetchRequest) -> FetchResponse:
        ...
        try:
            ...
            if request.start_hour is not None and request.end_hour is not None:
                start_time = datetime.combine(target_date, datetime.min.time()).replace(hour=request.start_hour)
                end_time = datetime.combine(target_date, datetime.min.time()).replace(hour=request.end_hour, minute=59, second=59)
            else:
            ...
            for msg in messages:
                try:
                    ...
                    # Check filters
                    if not self._matches_filters(msg, request.filters):
                        continue
                    ...
                    for attachment in msg.Attachments:
                        if attachment.FileName.endswith('.csv'):
                            ...
                            if attachment.FileName in request.attachment_rules:
                                ...
                                file_path = config.OUTPUT_DIR / new_filename
    ...
    def fetch_recent_emails(self, request: FetchRecentRequest) -> FetchResponse:
        ...
        cutoff_time = datetime.now() - timedelta(hours=request.hours)
    ...
    def _matches_filters(self, msg, filters: EmailFilter) -> bool:
        """Check if email matches the provided filters"""
        try:
            # Check sender filter
            if filters.sender:
                sender_email = getattr(msg, 'SenderEmailAddress', '')
                if not any(sender in sender_email for sender in filters.sender):
                    return False
            
            # Check subject filter
            if filters.subject_contains:
                subject = getattr(msg, 'Subject', '')
                if not any(keyword in subject for keyword in filters.subject_contains):
                    return False
...
async def health_check():
    outlook_connected = outlook_service is not None and outlook_service.is_connected()
