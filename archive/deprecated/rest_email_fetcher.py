...
class EmailFetcher:
    ...
    def __init__(self, config: ConfigLoader, save_dir: str = 'data/raw', log_file: str = 'fetch_csv.log'):
        ...
        self.service_url = email_service_config.get('url', 'http://localhost:8080')
        self.api_key = email_service_config.get('api_key', 'default-dev-key')
        self.timeout = email_service_config.get('timeout_seconds', 60)
        self.retry_attempts = email_service_config.get('retry_attempts', 2)
    ...
    def fetch(self, date_str: str) -> bool:
        ...
        try:
            ...
            # Make API call
            response_data = self._make_request('POST', '/emails/fetch', request_data)
            
            if response_data.get('success', False):
                processed_emails = response_data.get('processed_emails', 0)
                downloaded_files = response_data.get('downloaded_attachments', 0)
            ...
            return self._fallback_directory_scan(date_str)
    ...
    def fetch_hourly(self, date_str: str, hour: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Email service error: {e}")
            return self._fallback_directory_scan_hour(date_str, hour)
    ...
    def fetch_recent(self, hours: int) -> bool:
        ...
        try:
            ...
            response_data = self._make_request('POST', '/emails/fetch-recent', request_data)
            ...
            if response_data.get('success', False):
                ...
                # Also scan directory if enabled
                if self.config.directory_scan.get('enabled', False):
                    self._scan_directory_for_recent(hours)
            ...
            self.logger.error(f"Email service error: {e}")
            return self._fallback_directory_scan_recent(hours)
    ...
    def _fallback_directory_scan(self, date_str: str) -> bool:
    ...
    def _fallback_directory_scan_hour(self, date_str: str, hour: int) -> bool:
    ...
    def _fallback_directory_scan_recent(self, hours: int) -> bool:
    ...
    def _scan_directory_for_date(self, date_str: str):
        ...
        scan_path = Path(self.config.directory_scan.get('scan_path', ''))
        ...
        for file_path in scan_path.glob(pattern):
            ...
            if mod_time.date() == target_date:
                if file_path.name in self.config.attachment_rules:
                    ...
                    new_filename = f"{filename_base}__{timestamp}.csv"
    ...
    def _scan_directory_for_recent(self, hours: int):
