...
class SecureEmailSender:
    def __init__(self, config_path: str = None):
        ...
        self.senderemail = smtp_config.get('sender_email', '')

        # Authentication settings
        self.authmethod = smtp_config.get('auth_method', 'oauth')
        self.credentialsfile = smtp_config.get('credentials_file', 'credentials.json')
        self.tokenfile = smtp_config.get('token_file', 'token.json')

        # Make paths relative to config directory
        if not os.path.isabs(self.credentials_file):
            self.credentialsfile = os.path.join(self.config_dir, self.credentials_file)
        if not os.path.isabs(self.token_file):
            self.tokenfile = os.path.join(self.config_dir, self.token_file)
        ...
        if not creds or not creds.valid:
            ...
            if not creds:
                ...
                try:
                    ...
                    flow.redirecturi = 'urn:ietf:wg:oauth:2.0:oob'

                    auth_url, _ = flow.authorization_url(prompt='consent')
                    print("🔐 Please visit this URL to authorize the application:")
                    print(f"   {auth_url}")

                    authcode = input("Enter the authorization code: ")
    ...
    def get_service_account_credentials(self) -> Optional[service_account.Credentials]:
        ...
        try:
            ...
            delegatedcredentials = credentials.with_subject(self.sender_email)
        ...
        try:
            ...
            if self.auth_method in ['oauth', 'service_account']:
                ...
                if access_token:
                    ...
                    authstring = f'user={self.sender_email}\x01auth=Bearer {access_token}\x01\x01'
                    authstring_b64 = base64.b64encode(auth_string.encode()).decode()
...
{self.get_email_signature()}"""
    ...
    def send_test_email(self) -> bool:
        ...
        try:
            ...
            body = """This is a test email from the Charlie Reporting System using secure authentication.
