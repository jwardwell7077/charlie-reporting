...
def get_python_executable() -> str:
    ...
    return str(get_project_root() / ".venv" / "Scripts" / "python.exe")
...
def find_csv_files_by_type(data_dir: Optional[Path] = None) -> Dict[str, list]:
    ...
    """
    if data_dir is None:
        data_dir = get_generated_data_dir()
...
def get_one_file_per_type(data_dir: Optional[Path] = None) -> Tuple[Dict[str, str], bool]:
    ...
    """
    files_by_type = find_csv_files_by_type(data_dir)
    selected_files = {}
...
def get_relative_script_path(script_name: str, subdirectory: str = "") -> Path:
    ...
    """
    run_scripts_dir = get_demo_root() / "run_scripts"
    if subdirectory:
