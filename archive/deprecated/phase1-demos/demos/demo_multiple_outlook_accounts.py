...
def create_mock_outlook_with_multiple_accounts():
    ...
    account1.DeliveryStore.GetDefaultFolder.returnvalue = inbox1
    ...
    account2.DeliveryStore.GetDefaultFolder.returnvalue = inbox2
    ...
    account3.DeliveryStore.GetDefaultFolder.returnvalue = inbox3
    ...
    namespace.GetDefaultFolder.returnvalue = default_inbox
...
def demo_specific_account_selection():
    ...
    mockoutlook = create_mock_outlook_with_multiple_accounts()
    ...
    selectedinbox = fetcher.get_inbox_for_account(mock_outlook['namespace'])

    # Verify correct inbox was selected
    expectedinbox = mock_outlook['accounts'][1].DeliveryStore.GetDefaultFolder.return_value
    ...
    selectedinbox2 = fetcher2.get_inbox_for_account(mock_outlook['namespace'])
    expectedinbox2 = mock_outlook['accounts'][2].DeliveryStore.GetDefaultFolder.return_value
...
def demo_integration_with_fetching():
    ...
    with patch('win32com.client.Dispatch') as mock_dispatch:
        ...
        mock_app = Mock()
        mock_app.GetNamespace.returnvalue = mock_outlook['namespace']
        mock_dispatch.returnvalue = mock_app

        # Mock messages
        mock_messages = Mock()
        mock_messages.iter__ = Mock(return_value=iter([]))  # No messages for demo
        ...
        print("   Attempting to fetch emails for today...")

        try:
            # This would normally connect to real Outlook
            today = datetime.now().strftime('%Y-%m-%d')
            fetcher.fetch_for_timeframe(today)
            print("   ✓ Email fetch process completed successfully")
            print("   ✓ Used correct account inbox")
...
def demo_error_handling():
    ...
    error_namespace.GetDefaultFolder.returnvalue = default_inbox

    config = create_sample_config_with_account('test@company.com')
    fetcher = EmailFetcher(config)

    selectedinbox = fetcher.get_inbox_for_account(error_namespace)

    print(f"   Configured account: {config.email['outlook_account']}")
    print("   Error accessing accounts list")
    print(f"   Selected inbox: {selected_inbox.Name}")
    print("   ✓ Graceful fallback to default")

    print("\n2. Case sensitivity handling")
    mock_outlook = create_mock_outlook_with_multiple_accounts()

    # Test with different case
    configupper = create_sample_config_with_account('REPORTS@COMPANY.COM')
    fetcher_upper = EmailFetcher(config_upper)

    selectedinbox = fetcher_upper.get_inbox_for_account(mock_outlook['namespace'])
    expectedinbox = mock_outlook['accounts'][1].DeliveryStore.GetDefaultFolder.return_value

    print(f"   Configured account: {config_upper.email['outlook_account']}")
    print("   Available account: reports@company.com")
