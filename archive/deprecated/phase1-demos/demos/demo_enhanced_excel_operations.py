...
def demo_incremental_excel_updates():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Get sample data
        hour_9_data, hour_10_data, hour_11data = create_sample_hourly_data()

        date_str = '2025 - 07 - 28'

        print("1. Creating initial daily file with Hour 9 data...")
        dailyfile = writer.write_incremental(hour_9_data, date_str, '2025 - 07 - 28_09')
        print(f"   ✓ Created: {os.path.basename(daily_file)}")

        print("\n2. Adding Hour 10 data incrementally...")
        dailyfile = writer.write_incremental(hour_10_data, date_str, '2025 - 07 - 28_10')
        print(f"   ✓ Updated: {os.path.basename(daily_file)}")

        print("\n3. Adding Hour 11 data with new sheet (Productivity)...")
        dailyfile = writer.write_incremental(hour_11_data, date_str, '2025 - 07 - 28_11')
        print(f"   ✓ Updated: {os.path.basename(daily_file)}")

        # Verify the final file
        print("\n4. Analyzing final incremental file...")
        try:
            finaldata = pd.read_excel(daily_file, sheet_name=None)
...
def demo_custom_report_generation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        customfile = writer.write_custom(custom_data, 'custom_performance_report_20250728.xlsx')
        ...
        ondemandfile = writer.write_custom(sales_data, f'ondemand_sales_report_{timestamp}.xlsx')
        ...
        for file_path, report_type in [(custom_file, 'Performance'), (ondemand_file, 'Sales')]:
            try:
                ...
                totalrows = sum(len(df) for df in data.values())
...
def demo_end_of_day_summary():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        summaryfile = writer.write_summary(enhanced_data, 'eod_summary_20250728.xlsx')
        print(f"   ✓ Created: {os.path.basename(summary_file)}")

        print("\n2. Analyzing summary report structure...")
        try:
            # Read all sheets
            summarydata = pd.read_excel(summary_file, sheet_name=None)
            ...
            # Show data sheets
            datasheets = [s for s in summary_data.keys() if s != 'Summary']
            for sheet in data_sheets:
                df = summary_data[sheet]
                print(f"   {sheet}: {len(df)} rows")
                if 'Hour' in df.columns:
                    hourscovered = sorted(df['Hour'].unique())
...
def demo_error_handling_and_edge_cases():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        emptyresult = writer.write({}, '2025 - 07 - 28')
        print(f"   Empty data result: {empty_result}")

        print("\n2. Testing incremental update with empty data...")
        emptyincremental = writer.write_incremental({}, '2025 - 07 - 28', '2025 - 07 - 28_09')
        ...
        minimalfile = writer.write_custom(minimal_data, 'minimal_report.xlsx')
        ...
        metadatafile = writer.write_summary(metadata_only, 'metadata_only.xlsx')
