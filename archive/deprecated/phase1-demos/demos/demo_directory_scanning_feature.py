...
def create_sample_csv_files(directory: str):
    ...
    subfile = os.path.join(subdir, 'IB_Calls_Archive.csv')
    with open(sub_file, 'w') as f:
        f.write('Agent Name,Handle\nArchived Agent,50')

    submod_time = current_time - timedelta(hours=3)
    timestamp = sub_mod_time.timestamp()
    os.utime(sub_file, (timestamp, timestamp))

    created_files.append(('archive / IB_Calls_Archive.csv', sub_mod_time))
...
def demo_directory_scanning():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        scandir = os.path.join(temp_dir, 'incoming')
        savedir = os.path.join(temp_dir, 'processed')
        os.makedirs(scan_dir)
        os.makedirs(save_dir)

        print("1. Setting up demo environment...")
        createdfiles = create_sample_csv_files(scan_dir)
        ...
        # Check results
        processedfiles = os.listdir(save_dir)
        print("\n4. Results:")
        ...
        # Show final results
        finalfiles = os.listdir(save_dir)
        print("\n7. Final results:")
        ...
        for file in sorted(final_files)[:3]:  # Show first 3
            print(f"     {file}")
            if '__' in file:
                base, timestampext = file.split('__', 1)
                timestamppart = timestamp_ext.split('.')[0]
