...
# === Shared Components ===  # Install shared requirements: pip install -r ../../shared / requirements.txt

# === Service - Specific Dependencies ==
"""

    if service_name == "outlook - relay":
        requirements_content += """  # Outlook integration
...
"""
        ...
        requirements_content += """  # Scheduling
apscheduler>=3.10.0
croniter>=1.4.0
...
"""
        ...
        requirements_content += """  # Excel generation
openpyxl>=3.1.0
xlsxwriter>=3.1.0
xlrd>=2.0.0
...
class {service_name.replace("-", "").title()}Service(BaseService):
    ...
    def __init__(self):
        ...
        self.healthmonitor = HealthMonitor(self.config.service_name)
...
'''
    ...
    for model_file in service_config["business_models"]:
        modelname = model_file.replace(".py", "")
        model_content = '''"""
...
'''
    ...
    for service_file in service_config["business_services"]:
        servicefile_name = service_file.replace(".py", "")
        service_content = '''"""
...
'''
    ...
    dev_runner_content = '''#!/usr / bin / env python3
"""
...
'''
    ...
    # Service README
    readme_content = '''# {service_name.title()} Service
