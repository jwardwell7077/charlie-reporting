...
class Counter:
    ...
    def inc(self, amount: float = 1.0, **label_values):
        """Increment counter"""
        with self.lock:
            labelkey = self.make_label_key(label_values)
...
class Histogram:
    ...
    def get_stats(self, **label_values) -> Dict[str, float]:
        ...
        with self.lock:
            ...
            observations = self.observations[label_key]
...
class ServiceMetrics:
    ...
    def __init__(self, service_name: str):
        ...
        self.httprequests_total = Counter(
            name="http_requests_total",
            description="Total number of HTTP requests",
            labels=["service", "endpoint", "method", "status"]
        )

        self.httprequest_duration_seconds = Histogram(
        ...
        # === Business Operation Metrics ==
        self.businessoperations_total = Counter(
            name="business_operations_total",
            description="Total number of business operations",
            labels=["service", "operation", "status"]
        )

        self.businessoperation_duration_seconds = Histogram(
        ...
        self.servicehealth_status = Gauge(
            name="service_health_status",
            description="Service health status (1=healthy, 0=unhealthy)",
            labels=["service"]
        )

        self.serviceuptime_seconds = Gauge(
        ...
        # === Resource Metrics ==
        self.activeconnections = Gauge(
            name="active_connections",
            description="Number of active connections",
            labels=["service"]
        )

        self.memoryusage_bytes = Gauge(
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "business_operations_total": dict(self.business_operations_total.values),
            "service_health": self.service_health_status.get_value(service=self.service_name),
            "uptime_seconds": time.time() - self.start_time,
            "errors_total": dict(self.errors_total.values)
        }
...
class MetricsExporter:
    ...
    def export_prometheus_format(metrics: ServiceMetrics) -> str:
        ...
        for label_key, value in metrics.http_requests_total.values.items():
            if label_key:
