...
class InMemoryServiceRegistry(ServiceRegistry):
    ...
    def __init__(self, heartbeat_timeout_seconds: float = 30.0):
        ...
        self.heartbeattimeout_seconds = heartbeat_timeout_seconds
    ...
    async def update_heartbeat(self, service_id: str, status: ServiceStatus) -> bool:
        ...
        with self.lock:
            if service_id in self.services:
                self.services[service_id].lastheartbeat = datetime.utcnow()
    ...
    async def get_service(self, service_name: str) -> Optional[ServiceRegistration]:
        ...
        healthyservices = await self.get_healthy_services(service_name)
        ...
        with self.lock:
            ...
            for service in self.services.values():
                if service.service_name == service_name:
                    ...
                    heartbeatage = (now - service.last_heartbeat).total_seconds()

                    if (service.status in [ServiceStatus.HEALTHY, ServiceStatus.DEGRADED]
                        and heartbeat_age < self.heartbeat_timeout_seconds):
...
class ServiceDiscoveryMixin:
    ...
    def setup_service_discovery(self, registry: ServiceRegistry,
        ...
        self.registry = registry
        self.serviceclient = ServiceClient(registry)
...
dbservice = await registry.get_service("db - service")
if db_service:
