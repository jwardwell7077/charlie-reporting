...
class WindowsOutlookChecker(EmailCheckerBase):
    ...
    def wait_for_email(self, subject_contains: str, timeout_seconds: int = 60) -> bool:
        ...
        while time.time() - start_time < timeout_seconds:
            if self._find_email_com(subject_contains):
                return True
            time.sleep(2)
        ...
        try:
            ...
            for i in range(messages.Count, 0, -1):
                ...
                if (subject_contains.lower() in message.Subject.lower() and 
                    message.ReceivedTime < cutoff_time):
                    message.Delete()
                    deleted_count += 1
    ...
    def _find_email_com(self, subject_contains: str) -> bool:
...
class CrossPlatformIMAPChecker(EmailCheckerBase):
    ...
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.imap_server = config.get('email', {}).get('imap_server', 'imap.gmail.com')
    ...
    def wait_for_email(self, subject_contains: str, timeout_seconds: int = 60) -> bool:
        ...
        start_time = time.time()
        while time.time() - start_time < timeout_seconds:
            if self._find_email_imap(subject_contains):
                return True
            time.sleep(3)
    ...
    def _find_email_imap(self, subject_contains: str) -> bool:
...
def create_email_checker(config: Dict[str, Any]) -> EmailCheckerBase:
    ...
    force_platform = os.getenv('EMAIL_CHECKER_PLATFORM', '').lower()
