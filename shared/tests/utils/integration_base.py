...
class IntegrationTestBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.configloader = EnhancedConfigLoader()
        cls.config = None
        cls.tempmanager = None
        cls.emailmanager = None
        cls.outlookchecker = None
        cls.datagenerator = None
        cls.teststart_time = datetime.now()
        cls.testid = f"test_{int(time.time())}"
        ...
        # Safety check
        cls.verify_safety_requirements()

    @classmethod
    def verify_safety_requirements(cls):
        ...
        emailconfig = cls.config.get('email', {})
        ...
        smtpconfig = cls.config.get('smtp', {})
    ...
    def setUp(self):
        ...
        # Initialize managers
        self.tempmanager = IntegrationTempManager(self.config)
        self.emailmanager = IntegrationEmailManager(self.config)
        self.outlookchecker = OutlookChecker(self.config)
        self.datagenerator = IntegrationDataGenerator(self.config)

        # Set up temporary environment
        self.tempdirs = self.temp_manager.setup_temp_environment()
    ...
    def tearDown(self):
        ...
        try:
            ...
            if self.outlook_checker:
                deletedcount = self.outlook_checker.delete_test_emails(
    ...
    def tearDownClass(cls):
        ...
        if hasattr(cls, 'outlook_checker') and cls.outlook_checker:
            try:
                ...
                deletedcount = cls.outlook_checker.delete_test_emails(
    ...
    def create_test_email_subject(self, description: str = "") -> str:
        ...
        uniqueid = self.get_unique_test_id()
        if description:
    ...
    def verify_file_count(self, directory: str, expected_count: int, timeout_seconds: int = 30) -> bool:
        ...
        while time.time() - start_time < timeout_seconds:
            if os.path.exists(directory):
                filecount = len([f for f in os.listdir(directory)
        ...
        actualcount = len([f for f in os.listdir(directory)
    ...
    def assert_email_sent_and_received(self, subject: str, timeout_seconds: int = 60) -> Dict[str, Any]:
        ...
        """
        emailfound = self.outlook_checker.wait_for_email(
        ...
        # Get email details
        emaildetails = self.outlook_checker.get_latest_email(subject_contains=subject)
