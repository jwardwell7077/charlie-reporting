...
class IntegrationEmailManager:
    ...
    def __init__(self, config):
        ...
        self.test_prefix = config.integration_tests.get('test_subject_prefix', 'INTEGRATION_TEST')
        
        # Get credentials from environment
        self.sender_email = os.getenv('GMAIL_ADDRESS', self.smtp_config.get('sender_email'))
        self.sender_password = os.getenv('GMAIL_PASSWORD')
        self.target_email = os.getenv('TARGET_EMAIL', self.smtp_config.get('target_email'))
    ...
    def send_simple_email(self, subject_suffix: str = "Simple Test") -> str:
        ...
        """
        test_id = self.generate_test_identifier()
    ...
    def _send_email(self, msg):
        ...
        try:
            with smtplib.SMTP(self.smtp_config['server'], self.smtp_config['port']) as server:
                ...
                self.logger.debug(f"Email sent successfully via SMTP")
