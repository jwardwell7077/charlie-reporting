...
class EnhancedConfigLoader:
    ...
    def load_integration_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if config_path is None:
            ...
            config_path = os.path.join(os.getcwd(), 'tests', 'config', 'integration-config.toml')
    ...
    def _apply_environment_overrides(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        self.logger.debug(f"[{__name__}.EnhancedConfigLoader._apply_environment_overrides:84] Applying environment overrides")
        
        # Email configuration overrides
        if 'email' in config:
            env_sender = os.getenv('INTEGRATION_TEST_SENDER_EMAIL')
            if env_sender:
                config['email']['test_sender_address'] = env_sender
                self.logger.debug("Applied sender email from environment")
            
            env_receiver = os.getenv('INTEGRATION_TEST_RECEIVER_EMAIL')
            if env_receiver:
        ...
        if 'smtp' in config:
            env_server = os.getenv('INTEGRATION_TEST_SMTP_SERVER')
            if env_server:
                config['smtp']['server'] = env_server
                self.logger.debug("Applied SMTP server from environment")
            
            env_port = os.getenv('INTEGRATION_TEST_SMTP_PORT')
            if env_port:
                try:
            ...
            env_password = os.getenv('INTEGRATION_TEST_EMAIL_PASSWORD')
            if env_password:
        ...
        if 'outlook' in config:
            env_account = os.getenv('INTEGRATION_TEST_OUTLOOK_ACCOUNT')
            if env_account:
        ...
        if 'integration_tests' in config:
            env_enabled = os.getenv('INTEGRATION_TESTS_ENABLED')
            if env_enabled:
            ...
            env_timeout = os.getenv('INTEGRATION_TEST_TIMEOUT')
            if env_timeout:
                try:
        ...
        if 'directories' in config:
            env_temp_dir = os.getenv('INTEGRATION_TEST_TEMP_DIR')
            if env_temp_dir:
    ...
    def validate_config(self, config: Dict[str, Any], config_type: str = 'main') -> bool:
        ...
        if config_type == 'main':
            ...
            return self._validate_integration_config(config)
        else:
            raise ValueError(f"Unknown config type: {config_type}")
    
    def _validate_main_config(self, config: Dict[str, Any]) -> bool:
        """Validate main configuration."""
        required_sections = ['directories', 'logging']
        ...
        if 'directories' in config:
            required_dirs = ['data', 'raw', 'output', 'logs']
    ...
    def _validate_integration_config(self, config: Dict[str, Any]) -> bool:
        ...
        required_sections = ['integration_tests', 'email', 'smtp']
        ...
        if 'email' in config:
            required_email_fields = ['test_sender_address', 'test_receiver_address']
