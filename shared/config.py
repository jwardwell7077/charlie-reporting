...
class ConfigLoader:
    ...
    def load_config(
        ...
        service_config_file = config_path / "service.toml"
        if service_config_file.exists():
            service_config = toml.load(service_config_file)
            config_data.update(service_config)
        
        # 2. Load environment-specific config
        env_config_file = config_path / f"{env}.toml"
        if env_config_file.exists():
            env_config = toml.load(env_config_file)
...
class OutlookRelayConfig(BaseServiceConfig):
    ...
    archive_processed_emails: bool = Field(default=True, env="ARCHIVE_PROCESSED_EMAILS")
