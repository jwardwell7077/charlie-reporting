...
- **snake_case**: All variables, functions, and methods (e.g., `csv_files`, `processing_time`)
- **PascalCase**: Classes and interfaces (e.g., `ReportProcessingService`, `IConfigManager`)
- **UPPER_CASE**: Constants (e.g., `MAX_FILE_SIZE`, `DEFAULT_TIMEOUT`)
- **Descriptive Names**: Use clear, descriptive names over abbreviations
- **Consistent Patterns**: Follow established patterns for common variable types:
  - Files: `file_path`, `input_file_name`, `output_directory`
  - Collections: `csv_files`, `transformation_results`, `error_messages`
  - Time: `processing_time`, `start_time`, `created_at`
  - Config: `config_dict`, `transform_config`, `excel_settings`
  - IDs: `task_id`, `correlation_id`, `request_id`
  - Status: `task_status`, `processing_result`, `success_count`
- **Complete Standards**: See `services/report-generator/docs/NAMING_STANDARDS.md` for comprehensive guidelines

### Common Naming Fixes Required
