...
class ExcelWriter:
    ...
    def __init__(self, output_dir: str = 'data / output', log_file: str = 'report.log'):
        self.outputdir = Path(output_dir)  # Convert to Path object
    ...
    def write(self, report_data: dict, date_str: str) -> str:
        ...
        outputpath = self.output_dir / f'report_{date_str}.xlsx'
    ...
    def write_incremental(self, report_data: Dict, date_str: str, hour_str: str) -> str:
        ...
        dailyfile_path = self.output_dir / f'daily_{date_str}.xlsx'
        ...
        try:
            ...
            if daily_file_path.exists():
                ...
                existingdata = self.load_existing_data(daily_file_path)
            else:
                workbook = Workbook()
            ...
            mergeddata = self.merge_data(existing_data, report_data)

            # Write merged data back to workbook
            self.write_to_workbook(workbook, merged_data)
    ...
    def write_custom(self, report_data: Dict, filename: str) -> str:
        ...
        outputpath = self.output_dir / filename
        ...
        try:
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                ...
                if 'Report_Metadata' in enhanced_data:
                    metadatadf = enhanced_data['Report_Metadata'][0]
                ...
                for sheet_name, df_list in enhanced_data.items():
                    ...
                    summarystats = self.calculate_sheet_summary(combined_df, sheet_name)
    ...
    def write_to_workbook(self, workbook, data: Dict):
    ...
    def calculate_sheet_summary(self, df: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
    ...
    def get_time_range(self, df: pd.DataFrame) -> str:
        """Extract time range from DataFrame if date columns exist."""
        datecolumns = [col for col in df.columns if 'date' in col.lower() or 'time' in col.lower()]

        if not date_columns or df.empty:
            return 'N / A'

        try:
            datecol = date_columns[0]
